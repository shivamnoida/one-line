...
class HomeScreen extends StatefulWidget {
  ...
  static Future<void> loadData(bool reload, {bool fromModule = false}) async {
    ...
    // Load parcel data for parcel functionality
    Get.find<ParcelController>().getParcelCategoryList();
  }
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void _initializeGroceryAndLoadData() async {
    // Set grocery module first
    final splashController = Get.find<SplashController>();
    
    // Find and set grocery module
    await splashController.getModules();
    if (splashController.moduleList != null) {
      for (var module in splashController.moduleList!) {
        if (module.moduleType == AppConstants.grocery) {
    ...
    // Show refer bottom sheet if needed
    Get.find<SplashController>().getReferBottomSheetStatus();
    if((Get.find<ProfileController>().userInfoModel?.isValidForDiscount??false) && Get.find<SplashController>().showReferBottomSheet) {
      _showReferBottomSheet();
    }
            ...
            child: RefreshIndicator(
              ...
              child: ResponsiveHelper.isDesktop(context) ? WebNewHomeScreen(
                ...
                slivers: [
                      ...
                      child: Row(children: [
                        ...
                        InkWell(
                          ...
                          child: Container(
                            ...
                            child: Icon(CupertinoIcons.cube_box, size: 20, color: Theme.of(context).textTheme.bodyLarge!.color),
                          ),
                        ),
                  ...
                  SliverToBoxAdapter(
                    child: Center(child: SizedBox(
                      ...
                      child: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        // Always show grocery home screen
                        GroceryHomeScreen(),
                      ]),
                    )),
                  ),
                  ...
                  SliverToBoxAdapter(child: Center(child: GetBuilder<StoreController>(builder: (storeController) {
                    return Padding(
                      padding: EdgeInsets.only(bottom: ResponsiveHelper.isDesktop(context) ? 0 : 100),
                      child: PaginatedListView(
                        scrollController: _scrollController,
                        ...
                        itemView: ItemsView(
                          isStore: true,
                          items: null,
                          isFoodOrGrocery: true, // Always grocery
                          stores: storeController.storeModel?.stores,
                          padding: EdgeInsets.symmetric(
                            horizontal: ResponsiveHelper.isDesktop(context) ? Dimensions.paddingSizeExtraSmall : Dimensions.paddingSizeSmall,
                            vertical: ResponsiveHelper.isDesktop(context) ? Dimensions.paddingSizeExtraSmall : Dimensions.paddingSizeDefault,
